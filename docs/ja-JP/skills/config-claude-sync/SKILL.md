...
2. プロジェクトの `.claude/settings.json` を読み取る
   - 存在しない場合 → `{}` として扱う
3. `shared-hooks.json` の各hookエントリについて、3つの状態で重複チェックを行う:
   ...
   - 既存hookの `command` 文字列が `_detect_by` の正規表現パターンにマッチする場合:
     - マッチした既存hookの `command` と共通hookの `command` を比較する
     - 同一の場合 → 「同期済み」とみなす（スキップ）
     - 異なる場合 → 「変更あり」としてリストアップする（コマンドが更新されている）
   - マッチしない場合 → 「追加対象」としてリストアップする
4. 追加対象または変更対象があれば、ユーザーに提示して確認する:
   ```
   ## 未同期の Hooks
   ...
   ```
   - 追加がない場合は「### 追加」セクションを省略する
   - 変更がない場合は「### 変更」セクションを省略する
   - すべて同期済みの場合 → `すべての hooks は同期済みです。` と表示して Step 7 へ進む
5. 承認されたhooksを `.claude/settings.json` に反映する:
   - 挿入・更新前に `_id`・`_description`・`_detect_by` フィールドを除去する
   - **「追加対象」のhooks**: event/matcher 単位で既存の `hooks` オブジェクトにマージする（既存エントリは上書きしない）
   - **「変更あり」のhooks**: マッチした既存hookエントリを新しい `command` で上書きする
...
- Step 6b で hooks を追加または変更した場合のみ「同期した hooks」を表示する。各エントリに `(追加)` または `(変更)` を付記する
