...
ドキュメントの記述言語、および内部ドキュメント（仕様書、設計書など）の構成は、各プロジェクトの責任で定める。プロジェクトの `CLAUDE.md` または `.claude/rules/` に方針を明記すること。

補足: このルールは、`.claude/rules/shared/` および `.claude/skills/` 配下（`shared-claude-code` からのシンボリックリンク）には **適用しない**。これらは上流リポジトリで定義された英語マスタとしてそのまま維持する。
...
Claude Code はスキル本文中の `$ARGUMENTS` と `$<数字>`（`$1`, `$2`, ...）を、読み込み時に呼び出し引数で置換する — 引数を取らないスキルでも置換処理は走る。SKILL.md に埋め込むスクリプトにこれらのリテラルを書いてはならない。暗黙に（通常は空文字に）置換され、スクリプトが破壊される。位置フィールドを使わない代替手段を使うこと（例: `awk '{print $<n>}'` の代わりに `sed -n 's/^prefix //p'` や `cut`）。
